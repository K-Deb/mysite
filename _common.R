...
get_pubs <- function() {
  pubs <- gsheet::gsheet2tbl(
    url = 'https://docs.google.com/spreadsheets/d/1z5aqTjYXE1u5zwENUrjzLgWoL9Y0BMA2L-kZUlywark/edit?gid=0#gid=0')
  ...
  pubs$url_scholar <- ifelse(
    ...
    glue::glue('https://scholar.google.com/citations?view_op=view_citation&hl=en&user=fZF4M1MAAAAJ&citation_for_view=fZF4M1MAAAAJ:{pubs$id_scholar}')
  )
  return(pubs)
}
...
make_pub_list <- function(pubs, category) {
  ...
  # Safely return nothing if no publications in this category
  if (nrow(x) == 0) {
    return(HTML(""))
  }
  pub_list <- list()
...
#   }
#   
#   # Use markdownToHTML() to convert markdown to HTML
#   cite_html <- markdownToHTML(text = cite)
#   
#   return(htmltools::HTML(glue::glue(
#     '<div class="pub">
#     <div class="grid">
#     <div class="g-col-11"> {cite_html} </div>
#     </div>
#     </div>'
#   )))
# }
...
make_altmetric <- function(pub) {
  altmetric <- ""
  if (!is.na(pub$category) && pub$category == 'peer_reviewed') {
